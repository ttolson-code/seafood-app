...
const SpeciesCard = ({
  ...
  return (
    <li className="fishFinder-species-card">
      <Link
        className="fishFinder-species-card-link"
        to={`/fish-finder/species/${speciesId}`}
        onClick={() => setFilter(speciesId)}
      >
        <div className="fishFinder-species-card-content">
          {harvestType === 'Farmed' && (
            <div className="fishFinder-species-card-farmed-tag">
              <p>Farmed</p>
            </div>
          )}
          <img
            className="fishFinder-species-card-illustration"
